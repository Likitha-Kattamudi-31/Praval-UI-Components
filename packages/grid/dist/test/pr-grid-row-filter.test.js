...
describe("PrGridRowFilter", () => {
    it("should render the grid row filter", async () => {
        const el = await fixture(html `<pr-grid-row-filter .columns=${columns}></pr-grid-row-filter>`);
        expect(el).to.be.instanceOf(PrGridRowFilter);
    });
    it("renders and popluates the grid row filter", async () => {
        const el = await fixture(html `<pr-grid-row-filter
        .columns=${columns}
        .filters=${filters}
      ></pr-grid-row-filter>`);
        const openMenu = el.shadowRoot?.querySelector(".filter-button");
        openMenu.click();
        await elementUpdated(el);
        const columnOptions = el.shadowRoot?.querySelectorAll('select[data-v="firstName"] option');
        expect(columnOptions?.length).to.equal(3);
        const filterOperatorInput = el.shadowRoot?.querySelector("select.filterOperatorInput");
    ...
    it("should debounce the filter input", async () => {
      ...
      ></pr-grid-row-filter>`);
        ...
        openMenu.click();
        await elementUpdated(el);
        const filterInput = el.shadowRoot?.querySelector("div.filterValueField input");
        const debouncedOnFilterValueChangeSpy = vi.spyOn(el, "debouncedOnFilterValueChange");
        const onFilterValueChangeSpy = vi.spyOn(el, "onfilterValueChange");
        for (let i = 0; i < 5; i += 1) {
    ...
    describe("Filter type input", () => {
        ...
        ></pr-grid-row-filter>`);
            ...
            openMenu?.click();
            await elementUpdated(el);
            const dateInput = el.shadowRoot?.querySelector('div.filterValueField input[type="date"]');
        ...
        ></pr-grid-row-filter>`);
            ...
            openMenu?.click();
            await elementUpdated(el);
            const inputField = el.shadowRoot?.querySelector("div.filterValueField input");
