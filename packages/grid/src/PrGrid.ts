...
import type { Filter } from './components/PrGridRowFilter.js';
...
export class PrGrid extends LitElement {
  ...
  @property({ type: Array }) downloadMenuItems: PrGridDownloadMenuItemModel[] =
    [];
  ...
  @property({ type: String }) filterMaxDate = new Date().toISOString().split("T")[0];
      ...
      if (preservedColumns.length > 0) {
        ...
        const allColumnNamesFound =
          this.columns.every(column =>
            preservedColumns.some((pc:any) => pc.name === column.name)
          ) &&
          preservedColumns.every((pc:any) =>
