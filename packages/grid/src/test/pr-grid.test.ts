...
describe("PrGrid", () => {
  ...
  it("renders a grid", async () => {
    const el = await fixture<PrGrid>(html`<pr-grid></pr-grid>`);
  ...
  it("renders the correct number of rows", async () => {
    const el = await fixture<PrGrid>(
      html`<pr-grid .columns=${columns} .rows=${rows}></pr-grid>`,
    );
  ...
  it("renders no rows component state", async () => {
    const el = await fixture<PrGrid>(
      html`<pr-grid .columns=${columns} .rows=${[]}
  ...
  it("renders an pr-grid-row-filter", async () => {
    ...
    expect((<PrGridRowFilter>rowFilter).filterValueChangeDebounceTime).to.equal(
      1000,
    );
  });
  ...
  describe("PrGrid updateSort", () => {
    it("should update URL with sort, order, page, pageSize, filters", async () => {
      ...
      const el = await fixture<PrGrid>(html`
        <pr-grid
  ...
  describe("PrGrid LocalStorage Persistence", () => {
    ...
    it("should reset localStorage value if columns length does not match", async () => {
      const tempEl = await fixture<PrGrid>(
        html`<pr-grid .columns=${columns} localStorageID="testGrid"></pr-grid>`,
      );
      ...
      expect(storedColumns.map((c: any) => c.name)).to.deep.equal(
    ...
    it("should reorder columns based on table header flex order", async () => {
      ...
      shadowRoot.querySelectorAll = () => fakeHeaders as any;
