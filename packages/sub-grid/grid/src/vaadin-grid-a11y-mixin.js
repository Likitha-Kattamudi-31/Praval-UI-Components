...
export const A11yMixin = (superClass) =>
  class A11yMixin extends superClass {
    ...
    /** @protected */
    _testIdUpdateHeaderRows() {
      iterateChildren(this.$.header, (headerRow, index) => {
        headerRow.setAttribute('data-testid', `rowindex-${index + 1}`);
      });
    }
    ...
    _testIdUpdateFooterRows() {
      iterateChildren(this.$.footer, (footerRow, index) => {
        footerRow.setAttribute('data-testid', `rowindex-${this._a11yGetHeaderRowCount(this._columnTree) + this.size + index + 1}`);
      });
    }
     ...
     * @protected
     */
    _testIdUpdateRowRowindex(row, index) {
      row.setAttribute('data-testid', `rowindex-${index + this._a11yGetHeaderRowCount(this._columnTree) + 1}`);
    }
