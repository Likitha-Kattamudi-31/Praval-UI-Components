...
export const ColumnResizingMixin = (superClass) =>
  class ColumnResizingMixin extends superClass {
    ...
    async ready() {
      ...
      window.addEventListener('resize', this._onBreakPointResize.bind(this));
      ...
      // run once to set the initial responsive column widths
      setTimeout(() => {
        this._onBreakPointResize();
      }, 0);
    }

    /** @private */
    _onBreakPointResize(e) {
        this.$.scroller.toggleAttribute('column-resizing', true);
        const windowWidth = window.innerWidth;
        const columns = this.getElementsByTagName('vaadin-grid-column');
        const responsiveColumns = Array.from(columns).filter((column) => column.responsive);
        responsiveColumns.forEach((column) => {
          const size = column.responsive.findLast(([width]) => parseInt(width) <= windowWidth);
